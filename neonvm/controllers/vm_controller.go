...
func (r *VMReconciler) doReconcile(ctx context.Context, vm *vmv1.VirtualMachine) error {
	...
	if vm.Status.PodName != "" && vm.Status.MemoryProvider == nil {
		log.Error(nil, "Setting default MemoryProvider for VM", "MemoryProvider", r.Config.DefaultMemoryProvider)
	...
	case vmv1.VmPending:
		// Generate runner pod name and set desired memory provider. After this, we know that
		// MemoryProvider will be set, because earlier (above the 'switch vm.Status.Phase'), we set
		// the MemoryProvider iff PodName was set but it wasn't.
	...
	case vmv1.VmScaling:
		...
		case vmv1.MemoryProviderVirtioMem:
			...
			ramScaled = currentTotalSize.Value() == goalTotalSize.Value()
