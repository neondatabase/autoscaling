...
::respawn:su -p vm-informant -c 'RUST_LOG=info /usr/local/bin/vm-monitor --addr 0.0.0.0:10301 --cgroup=neon-postgres{{if .FileCache}} --pgconnstr="dbname=postgres user=cloud_admin sslmode=disable"{{end}}'
