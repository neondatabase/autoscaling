...
func (c *PoolClient) PoolName() string {
	...
	if c.network.Name == UnnamedNetwork {
		return slug
	}

	return fmt.Sprintf("%s-%s", c.network.Name, slug)
}

func (c *PoolClient) Get(ctx context.Context) (*vmv1.IPPool, error) {
	pool, err := c.vmClient.NeonvmV1().IPPools(c.network.Namespace).Get(ctx, c.PoolName(), metav1.GetOptions{})
	if err == nil {
		return pool, nil
	}
	...
	ipPool, err := c.vmClient.NeonvmV1().IPPools(c.network.Namespace).Create(ctx, newPool, metav1.CreateOptions{})
	if err != nil {
