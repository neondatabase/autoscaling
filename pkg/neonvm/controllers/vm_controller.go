...
func (r *VMReconciler) doReconcile(ctx context.Context, vm *vmv1.VirtualMachine) error {
	...
	case vmv1.VmPending:
		...
		if len(vm.Status.PodName) == 0 {
			...
			// Set status.memoryProvider equal to VirtioMem -- either it's:
			// * not set, in which case we need to do this for backwards compatibility / rollback safety;
			// * it's already set to VirtioMem; or
			// * or it's set to DIMMSlots (we would have bailed out above).
			//
			//nolint:staticcheck // We know it's deprecated
