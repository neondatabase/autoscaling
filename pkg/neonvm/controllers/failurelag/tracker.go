...
func (t *Tracker[T]) processQueue(now time.Time) {
	...
	for ; i < len(t.refreshQueue); i++ {
		...
		if event.ts.Sub(t.lastFailure[event.key]) < t.period {
			// There was a failure in between
			t.degradedRetried[event.key] = struct{}{}
		} else {
			// This was the last event for this key
...
func (t *Tracker[T]) DegradedRetried() []T {
	...
	t.processQueue(t.Now())
	keys := make([]T, 0, len(t.degradedRetried))
	for k := range t.degradedRetried {
		keys = append(keys, k)
	}
	return keys
}
...
func (t *Tracker[T]) DegradedNotRetried() []T {
	...
	t.processQueue(t.Now())
	keys := make([]T, 0, len(t.degradedNotRetried))
	for k := range t.degradedNotRetried {
		keys = append(keys, k)
	}
	return keys
}
