...
type Client[E any] struct {
	...
	SerializeBatch  func(events []E) ([]byte, SimpleError)
}
...
type BaseClient interface {
	LogFields() zap.Field
	Send(ctx context.Context, payload []byte, traceID string) SimpleError
}
...
// SimpleError is an extension of the standard 'error' interface that provides a safe-for-metrics
...
func WrapSerialize[E any](wrapper func([]byte) ([]byte, SimpleError), base func([]E) ([]byte, SimpleError)) func([]E) ([]byte, SimpleError) {
	return func(events []E) ([]byte, SimpleError) {
		bs, err := base(events)
		if err != nil {
			return nil, err
		}
...
func JSONMarshalBatch[V any](value V) ([]byte, SimpleError) {
...
func JSONLinesMarshalBatch[E any](events []E) ([]byte, SimpleError) {
	buf := bytes.Buffer{}
...
func GZIPCompress(payload []byte) ([]byte, SimpleError) {
	buf := bytes.Buffer{}
