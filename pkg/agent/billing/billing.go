...
type vmMetricsHistory struct {
	...
	totalIngressBytes vmv1.NetworkBytes
	totalEgressBytes  vmv1.NetworkBytes
}
...
func (mc *MetricsCollector) Run(
	...
	state.collect(ctx, logger, store, mc.metrics)
...
func collectMetricsForVM(
	...
	byteCounts := &vmv1.VirtualMachineNetworkUsage{
		IngressBytes: 0,
		EgressBytes:  0,
	}
	if vm.Spec.EnableNetworkMonitoring != nil && *vm.Spec.EnableNetworkMonitoring {
		...
		if err != nil {
			...
			var rootErr string
			switch e := err.(type) { //nolint:errorlint // we want to know the type
			case vmv1.NetworkUsageJSONError:
				rootErr = "JSON unmarshaling"
			case vmv1.NetworkUsageStatusCodeError:
	...
	metricsChan <- vmMetricsKV{
		...
		// Note: Usually, vmMetricInstant stores deltas for the byte counts. But the API
		// returns totals, which get recorded here. They will get subtracted off from the previous
		// instant in the second half of the collection loop.
		value: vmMetricsInstant{
			cpu:          *vm.Status.CPUs,
...
func (s *metricsState) collect(ctx context.Context, logger *zap.Logger, store VMStoreForNode, metrics PromMetrics) {
	...
	for _, vm := range vmsOnThisNode {
		...
		go collectMetricsForVM(ctx, logger, vm, metricsChan, &metrics)
		metricsToCollect += 1
	}
	...
	for i := 0; i < metricsToCollect; i++ {
		select {
		case <-ctx.Done():
			logger.Error("Timed out collecting metrics", zap.Error(ctx.Err()))
			break
		case kv := <-metricsChan:
			key, presentMetrics := kv.key, kv.value
