...
// helper function to use (*XactMap).Iter() to simply build a list of the key/value pairs returned.
func list[K comparable, V any](m *state.XactMap[K, V], length int) []pair[K, V] {
	var items []pair[K, V]

	m.Iter(func(lc state.LoopControl, key K, value V) state.LoopControlFlow {
		if len(items) >= length {
			return lc.Break()
		}
