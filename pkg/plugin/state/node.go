...
// IterPods iterates through the pods on the Node, calling (*XactMap).Iter() with the provided
// function.
//
// Refer to (*XactMap).Iter() for more information on how to use this method.
func (n *Node) IterPods(forEach func(LoopControl, types.UID, Pod) LoopControlFlow) {
	n.pods.Iter(forEach)
}

// IterMigratablePods iterates through the migratable pods on the Node, calling (*XactMap).Iter()
// with the provided function.
//
// This method is provided as a specialized version of (*Node).IterPods() in order to support more
// efficient look-ups when trying to balance nodes.
//
// Refer to (*XactMap).Iter() for more information on how to use this method.
func (n *Node) IterMigratablePods(forEach func(LoopControl, types.UID, Pod) LoopControlFlow) {
	n.migratablePods.Iter(func(lc LoopControl, uid types.UID, _ struct{}) LoopControlFlow {
		pod, ok := n.pods.Get(uid)
		if !ok {
			panic(fmt.Sprintf("pod with UID %s preset in migratablePods map but not pods map", uid))
		}

		return forEach(lc, uid, pod)
	})
}
