...
func watchConfig[T any](metrics watch.Metrics, config Config) watch.Config {
	...
	return watch.Config{
		...
		RetryRelistAfter: util.NewTimeRange(time.Second, int(config.RetryRelistMin.Seconds), int(config.RetryRelistMax.Seconds)),
		RetryWatchAfter:  util.NewTimeRange(time.Second, int(config.RetryWatchMin.Seconds), int(config.RetryWatchMax.Seconds)),
	}
}
