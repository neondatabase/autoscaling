...
type Config struct {
	...
	StartupEventHandlingTimeout duration.Duration `json:"startupEventHandlingTimeout"`

	// kubernetes objects.
	K8sCRUDTimeout duration.Duration `json:"k8sCRUDTimeout"`

	// successive patch operations on a VirtualMachine object.
	PatchRetryWait duration.Duration `json:"patchRetryWait"`
	
	UpdateTimeout duration.Duration `json:"updateTimeout"`
	
	RetryRelistMin duration.Duration `json:"retryRelistMin"`
	RetryRelistMax duration.Duration `json:"retryRelistMax"`
	RetryWatchMin duration.Duration `json:"retryWatchMin"`
	RetryWatchMax duration.Duration `json:"retryWatchMax"`
...
func (c *Config) validate() (string, error) {
	...
	if c.StartupEventHandlingTimeout.Seconds == 0 {
		return "startupEventHandlingTimeout.seconds", errors.New("value must be > 0")
	}

	if c.K8sCRUDTimeout.Seconds == 0 {
		return "k8sCRUDTimeout.seconds", errors.New("value must be > 0")
	}

	if c.PatchRetryWait.Seconds == 0 {
		return "patchRetryWait.seconds", errors.New("value must be > 0")
	}
	
	if c.UpdateTimeout.Seconds == 0 {
		return "updateTimeout.seconds", errors.New("value must be > 0")
	}
	
	if c.RetryRelistMin.Seconds == 0 {
		return "retryRelistMin.seconds", errors.New("value must be > 0")
	}
	
	if c.RetryRelistMax.Seconds == 0 {
		return "retryRelistMax.seconds", errors.New("value must be > 0")
	}
	
	if c.RetryRelistMax.Seconds < c.RetryRelistMin.Seconds {
		return "retryRelistMax.seconds", errors.New("value must be >= retryRelistMin.seconds")
	}
	
	if c.RetryWatchMin.Seconds == 0 {
		return "retryWatchMin.seconds", errors.New("value must be > 0")
	}
	
	if c.RetryWatchMax.Seconds == 0 {
		return "retryWatchMax.seconds", errors.New("value must be > 0")
	}
	
	if c.RetryWatchMax.Seconds < c.RetryWatchMin.Seconds {
		return "retryWatchMax.seconds", errors.New("value must be >= retryWatchMin.seconds")
	}
